...
def transition_model(corpus, page, damping_factor):
    ...
    # to it)
    damping_prob_random = (1 - ___) / len(corpus)
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for i in range(n - 1):
        ...
        model = ___(corpus, next_page, damping_factor)

        # Choose a page at random considering probabilities given by transition model
        next_page = random.choices(list(___), weights=model.values(), k=1).pop()
        ...
        for page in ___.keys():
            ...
            for page_i, pages in corpus.items():
                ...
                if page in pages:
                    ...
                    probability += ___[page_i] / len(___)
