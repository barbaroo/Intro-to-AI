    ...
    for have_trait in powerset(names):
        ...
        fails_evidence = any(
            (people[person]["trait"] is not None and
             people[person]["trait"] != (person in ___))
            for person in names
        )
        if ___:
            continue
        ...
        for one_gene in powerset(names):
            for two_genes in powerset(names - one_gene):
                ...
                p = joint_probability(people, ___, ___, ___)
                update(___, one_gene, two_genes, have_trait, p)
...
def joint_probability(___, one_gene, two_genes, have_trait):
    """
    ...
    """
    probability = 1
    ___ = people.keys() - (one_gene | two_genes) # Set of people with no genes
    ...
    for person in zero_gene:
        ...
        if people[person]["mother"] is None:
            ...
            if mother in zero_gene and father in zero_gene:
                ...
                probability *= (1 - PROBS[___]) ** 2
            if mother in zero_gene and father in ___:
                # p that person doesn't get gene from mother is 0.99
            ...
            if mother in zero_gene and father in ___:
                ...
                probability *= (1 - PROBS["mutation"]) * PROBS[___]

            if mother in ___ and father in ___:
            ...
            if mother in two_genes and father in one_gene:
                ...
                probability *= PROBS["mutation"] * ___
        ...
        probability *= PROBS["trait"][0][person in ___]
    ...
    for person in one_gene:
        ...
        if people[person]["mother"] is None:
            ...
            if mother in zero_gene and father in zero_gene:
                ...
                probability *= PROBS["mutation"] * (1 - PROBS["mutation"]) + PROBS["mutation"] * (1 - ___[___])
            if mother in zero_gene and father in one_gene:
                ...
                # p that person doesn't get gene from mother is 0.99
                probability *= PROBS["mutation"] * ___ + (1 - PROBS["mutation"]) * ___
            if mother in ___ and father in ___:
                # p that person gets gene from mother is 0.01
                # p that person doesn't get gene from father is 0.01
                # p that person gets gene from father is 0.99
                # p that person doesn't get gene from mother is 0.99
                probability *= PROBS["mutation"] ** 2 + (1 - PROBS["mutation"]) ** ___
            ...
            if mother in one_gene and father in zero_gene:
                ...
                # p that person doesn't get gene from mother is 0.5
                probability *= (1 - PROBS["mutation"]) * ___ + PROBS["mutation"] * ___
            if mother in one_gene and father in one_gene:
            ...
            if mother in ___ and father in two_genes:
                # p that person gets gene from mother is 0.5
                # p that person doesn't get gene from father is 0.01
                # p that person gets gene from father is 0.99
                # p that person doesn't get gene from mother is 0.5
                probability *= 0.5 * ___ + (1 - PROBS["mutation"]) * 0.5
            ...
            if mother in two_genes and father in one_gene:
                ...
                # p that person doesn't get gene from mother is 0.01
                probability *= (1 - PROBS["mutation"]) * ___ + PROBS["mutation"] * 0.5
            if mother in two_genes and father in two_genes:
                # p that person gets gene from mother is 0.99
                # p that person doesn't get gene from father is 0.01
                # p that person gets gene from father is 0.99
                # p that person doesn't get gene from mother is 0.01
                probability *= (1 - PROBS["mutation"]) * ___ + (1 - PROBS["mutation"]) * ___

        # Multiply by the probability that the person has trait with one gene
        probability *= PROBS["trait"][1][person in ___]
    ...
    for person in ___:
        ...
        if people[person]["mother"] is None:
            ...
            if mother in one_gene and father in two_genes:
                # p that person gets gene from mother is 0.5
                # p that person gets gene from father is 0.99
                probability *= 0.5 * (___ - ___)
            if mother in ___ and father in zero_gene:
                # p that person gets gene from mother is 0.99
                # p that person gets gene from father is 0.01
                probability *= (1 - PROBS["mutation"]) * ___
            if mother in two_genes and father in one_gene:
                # p that person gets gene from mother is 0.99
                # p that person gets gene from father is 0.5
                probability *= (1 - PROBS["mutation"]) * 0.5
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        ...
        if person in ___:
            probabilities[person][___][1] += p
        elif person in ___:
            probabilities[person][___][2] += p
        else:
            probabilities[person][___][0] += p

        # if person has or hasn't trait, add the joint probability
        probabilities[person]["trait"][person in ___] += p
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        for i in range(0, 3):
            probabilities[person]["gene"][i] /= ___

        ___ = sum(probabilities[person]["trait"].values())
